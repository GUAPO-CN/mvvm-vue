 ...
 * @param {*} vm 
 * 功能：
 *      1、解析模板，判断是文字还是标签，文字直接处理
 *      2、解析标签属性，判断是否为特地指令，特地指令处理，
 *      3、为1、2步解析后的结果创建一个订阅者，订阅者会自动将自己添加到对应的订阅器中
 *      4、订阅者都有一个callback函数，用于执行被劫持的数据发生变动后通知订阅者，订阅者有一个update函数用于执行callback函数进而更新视图
 */
